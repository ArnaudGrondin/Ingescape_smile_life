...
def on_agent_event_callback(event, uuid, name, event_data, my_data):
    try:
        ...
        if event == igs.PEER_ENTERED:
            if name.startswith("Maitre"):
                agent.maitre = name
                igs.info(f"Connexion au maître détectée : {name}")
            elif name == "Whiteboard":
                agent.whiteboard = name
                igs.info(f"Connexion au whiteboard détectée : {name}")
    except:
        print(traceback.format_exc())

def on_service_callback(sender_agent_name, sender_agent_uuid, service_name, arguments, token, my_data):
    """
    Callback pour gérer les services appelés par le maître.
    """
    try:
        agent = my_data
        assert isinstance(agent, Joueur)

        if service_name == "setHand":
            cartes = arguments[0]
            agent.set_hand(cartes)
        elif service_name == "addCard":
            carte = arguments[0]
            agent.ajouter_carte(carte)
        elif service_name == "yourTurn":
            agent.notifier_tour()
        elif service_name == "getScore":
            score = agent.calculer_score()
            igs.output_set_int("score", score)
            igs.info(f"Score envoyé : {score}")
    except:
        print(traceback.format_exc())
...
if __name__ == "__main__":
    ...
    igs.observe_agent_events(on_agent_event_callback, joueur)
    igs.service_init("setHand", on_service_callback, joueur)
    igs.service_arg_add("setHand", "cartes", igs.STRING_T_ARRAY)
    igs.service_init("addCard", on_service_callback, joueur)
    igs.service_arg_add("addCard", "carte", igs.STRING_T)
    igs.service_init("yourTurn", on_service_callback, joueur)
    igs.service_init("getScore", on_service_callback, joueur)
