...
def on_service_callback(sender_agent_name, sender_agent_uuid, service_name, arguments, token, my_data):
    """
    Callback pour gérer les services appelés par le maître ou les joueurs.
    """
    try:
        agent = my_data
        assert isinstance(agent, Whiteboard)

        if service_name == "updateGameState":
            pioche_restante, defausse_top = arguments
            agent.update_game_state(pioche_restante, defausse_top)
        elif service_name == "showScores":
            scores = arguments[0]
            agent.show_scores(scores)
        elif service_name == "clear":
            agent.clear()
    except:
        print(traceback.format_exc())
...
if __name__ == "__main__":
    ...
    igs.observe_agent_events(on_agent_event_callback, agent)
    igs.service_init("updateGameState", on_service_callback, agent)
    igs.service_arg_add("updateGameState", "pioche_restante", igs.INTEGER_T)
    igs.service_arg_add("updateGameState", "defausse_top", igs.STRING_T)
    igs.service_init("showScores", on_service_callback, agent)
    igs.service_arg_add("showScores", "scores", igs.STRING_T_ARRAY)
    igs.service_init("clear", on_service_callback, agent)
